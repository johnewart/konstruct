...
import { TSNode } from './codebaseOutline';
  ...
  function walk(node: TSNode | null) {
    ...
    if (IMPORT_EXPORT_NODES.has(type)) {
      if (type === 'import_statement' || type === 'import_clause') {
        // Find the string literal for the module path
        let modulePath = '';
        let identifier = '';
        ...
        for (let i = 0; i < node.childCount; i++) {
          ...
          if (child.type === 'string_literal') {
            // Extract the module path from string literal
            const pathText = sourceCode.slice(child.startIndex, child.endIndex);
            modulePath = pathText.slice(1, -1); // Remove quotes
          } else if (child.type === 'identifier' && modulePath === '') {
            // Extract the identifier being imported
        ...
        if (modulePath) {
          ...
          if (modulePath.startsWith('./') || modulePath.startsWith('../')) {
            ...
            // Use the original path for the graph, but track resolved path for completeness
          ...
          // Add target module as a node if not already present
        ...
        if (identifier) {
          graph.edges.push({
            source: filePath,
            target: filePath, // export default is to the same file
            type: 'export',
...
// Import getParser from codebaseOutline
import { getParser } from './codebaseOutline';

// Re-export types for convenience
export { DependencyNode, DependencyEdge, DependencyGraph };
